...
long io_blocksize(int fd) {
    ...
    long blksize = st.st_blksize;
    if (blksize <= 0) {
        return page_size;
    }

    long long common_multiple = (long long)page_size / gcd(page_size, blksize) * blksize;
    
    if (common_multiple > MAX_BUFFER_SIZE) {
        return MAX_BUFFER_SIZE;
    }

    return (long)common_multiple;
}
...
int main(int argc, char *argv[]) {
    ...
    long buffer_size = io_blocksize(fd);
